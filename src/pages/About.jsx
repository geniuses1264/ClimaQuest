...
export default function About() {
  ...
  return (
    ...
    <main className="min-h-screen w-full flex items-center justify-center p-4 sm:p-6 md:p-10 bg-gradient-to-br from-slate-900 to-sky-800 overflow-x-hidden">
      ...
      <motion.div
        className="pointer-events-none absolute -z-10 rounded-full mix-blend-screen opacity-30
                   right-0 top-0
                   w-[60vw] h-[60vw] sm:w-[48vw] sm:h-[48vw] md:w-[34vw] md:h-[34vw] lg:w-[28vw] lg:h-[28vw]"
        style={{
          background:
            "radial-gradient(circle at 30% 30%, rgba(56,189,248,0.45), transparent 30%)",
          transform: "translate(18%, -12%)", // visual offset via transform (doesn't affect layout width)
        }}
      ...
      <motion.div
        className="pointer-events-none absolute -z-10 rounded-full mix-blend-screen opacity-25
                   left-0 bottom-0
                   w-[70vw] h-[70vw] sm:w-[52vw] sm:h-[52vw] md:w-[38vw] md:h-[38vw] lg:w-[30vw] lg:h-[30vw]"
        style={{
          background:
            "radial-gradient(circle at 70% 70%, rgba(14,165,233,0.35), transparent 30%)",
          transform: "translate(-14%, 10%)", // visual offset via transform (keeps layout stable)
        }}
        initial={{ scale: 0.9, opacity: 0 }}
      ...
      <section className="relative w-full max-w-5xl">
        <div className="bg-white/6 backdrop-blur-lg border border-white/10 rounded-2xl shadow-xl p-4 sm:p-6 md:p-10 overflow-hidden">
          <header className="text-center mb-5 sm:mb-8 md:mb-10 px-2">
            <motion.h1
            ...
            <motion.p
              className="mt-2 text-sm sm:text-base md:text-lg text-sky-100/90 max-w-2xl mx-auto px-2"
            ...
            <motion.p
              className="mt-4 text-xs sm:text-sm md:text-base text-slate-200/80 max-w-3xl mx-auto px-2 leading-relaxed"
          ...
          {/* About Section */}
          <motion.section
            className="bg-white/6 backdrop-blur-md border border-white/8 rounded-xl p-4 sm:p-5 md:p-6 mb-5 sm:mb-6 md:mb-6 hover:shadow-sky-400/20 transition-shadow"
            initial={{ scale: 0.99, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 0.6 }}
            aria-labelledby="about-title"
          >
            <h2 id="about-title" className="text-lg sm:text-xl md:text-2xl font-semibold text-white/95">About ClimaQuest</h2>
            <p className="mt-3 text-sm sm:text-base md:text-base text-slate-200/80">
          ...
          {/* FAQ */}
          <section className="mb-5 sm:mb-6 md:mb-6 px-1 sm:px-0">
            <h3 className="text-base sm:text-lg md:text-lg font-semibold text-white/95 mb-3">Frequently Asked Questions</h3>
              ...
              {FAQ_ITEMS.map((it, i) => {
                ...
                return (
                  ...
                  >
                    <motion.button
                      onClick={() => toggle(i)}
                      className="w-full text-left px-3 sm:px-4 py-3 flex items-center justify-between gap-4"
                      aria-expanded={isOpen}
                      aria-controls={`faq-${i}`}
                    >
                      <span className="text-sm sm:text-base md:text-base text-white/95 font-medium">{it.q}</span>
                      <span className={`text-sky-200/90 text-xs sm:text-sm transition-transform ${isOpen ? "rotate-45" : "rotate-0"}`}>
                    ...
                    >
                      <div className="py-3 text-sm sm:text-sm md:text-sm text-slate-200/80">
                        {it.a}
                      </div>
                    </motion.div>
                  </div>
          ...
          {/* Contact */}
          <motion.section
            className="bg-white/6 backdrop-blur-md border border-white/8 rounded-xl p-4 sm:p-5 md:p-6 mb-5 sm:mb-6 flex flex-col md:flex-row items-center justify-between gap-3"
            initial={{ y: 6, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.6 }}
          >
            <div className="text-center md:text-left">
              <h4 className="text-base sm:text-lg md:text-lg font-semibold text-white/95">Contact Us</h4>
              <p className="mt-2 text-xs sm:text-sm md:text-sm text-slate-200/80">Have feedback, suggestions, or questions? We’d love to hear from you.</p>
              <a
